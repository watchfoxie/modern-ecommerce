...
export const userService = {
  async getMe(): Promise<UserProfileDto> {
    const response = await api.get<UserProfileDto>('/user-service/users/me')
    return response.data
  },

  async updateMe(request: UpdateUserProfileRequest): Promise<UserProfileDto> {
    const response = await api.put<UserProfileDto>('/user-service/users/me', request)
    return response.data
  },

  async addAddress(request: UpsertUserAddressRequest): Promise<UserProfileDto> {
    const response = await api.post<UserProfileDto>('/user-service/users/me/addresses', request)
    return response.data
  },

  async updateAddress(addressIndex: number, request: UpsertUserAddressRequest): Promise<UserProfileDto> {
    const response = await api.put<UserProfileDto>(`/user-service/users/me/addresses/${addressIndex}`, request)
    return response.data
  },

  async deleteAddress(addressIndex: number): Promise<void> {
    await api.delete(`/user-service/users/me/addresses/${addressIndex}`)
  },
