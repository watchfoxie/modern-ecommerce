...
public class ActuatorInfoContributorConfiguration {
	...
	@Bean
	InfoContributor userServiceInfoContributor(Environment environment, @Value("${server.port}") String serverPort) {
		return builder -> builder
				.withDetail("app", appDetails(environment, serverPort))
				.withDetail("service", serviceDetails("User profile bootstrap", List.of("eureka", "mongodb")));
	}
	...
	private LinkedHashMap<String, Object> appDetails(Environment environment, String serverPort) {
		...
		app.put("name", environment.getProperty("spring.application.name", "user-service"));
