...
public class ActuatorInfoContributorConfiguration {
	...
	@Bean
	InfoContributor cartServiceInfoContributor(Environment environment, @Value("${server.port}") String serverPort) {
		return builder -> builder
				.withDetail("app", appDetails(environment, serverPort))
				.withDetail("service", serviceDetails("Shopping cart bootstrap", List.of("eureka", "mongodb")));
	}
	...
	private LinkedHashMap<String, Object> appDetails(Environment environment, String serverPort) {
		...
		app.put("name", environment.getProperty("spring.application.name", "cart-service"));
