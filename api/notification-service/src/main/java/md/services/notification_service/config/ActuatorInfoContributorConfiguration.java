...
public class ActuatorInfoContributorConfiguration {
	...
	@Bean
	InfoContributor notificationServiceInfoContributor(Environment environment, @Value("${server.port}") String serverPort) {
		return builder -> builder
				.withDetail("app", appDetails(environment, serverPort))
				.withDetail("service",
						serviceDetails("Notification consumption and inspection", List.of("eureka", "rabbitmq", "smtp")));
	}
	...
	private LinkedHashMap<String, Object> appDetails(Environment environment, String serverPort) {
		...
		app.put("name", environment.getProperty("spring.application.name", "notification-service"));
